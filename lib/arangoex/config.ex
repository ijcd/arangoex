defmodule Arangoex.Config do

  @moduledoc false

  # Generates the configuration for an endpoint.
  # It starts with the defaults for a given environment
  # and then merges in the common config from the arangoex config root,
  ...
  1) Defaults are pulled from `Arangoex.Config.Defaults`
  2) Common values set via e.g `config :arangoex` are merged in.
  3) Keys set on the individual api e.g `config :arangoex, :replication` are merged in
  ...
  def build_base(endpoint, overrides \\ %{}) do
    defaults = Arangoex.Config.Defaults.get(endpoint)
    common_config = Application.get_all_env(:arangoex) |> Map.new |> Map.take(@common_config)
    endpoint_config = Application.get_env(:arangoex, endpoint, []) |> Map.new
